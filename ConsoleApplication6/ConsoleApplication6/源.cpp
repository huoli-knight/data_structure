...
void cal(dat1 E)
{
    ...
    SqStack S;
	S.base1=(double (*))malloc(MAXSIZE*sizeof(double));
	S.base2=(double (*))malloc(MAXSIZE*sizeof(double));
    S.top_real=S.base1;
	S.top_imaginary=S.base2;
    S.Size=2*MAXSIZE;
	for(;E->real[0]!='\0'||E->imaginary[0]!='\0';)
    {
        ...
        else if(E->real[0]=='*')
        {
            ...
            a[0]=*S.top_real;
			a[1]=*S.top_imaginary;
            *S.top_real=(a[0]*b[0])-(a[1]*b[1]);
			*S.top_imaginary=(a[0]*b[1])+(a[1]*b[0]);
            S.top_real++;
			S.top_imaginary++;
        }
        else if(E->real[0]=='/')
        {
            ...
            a[0]=*S.top_real;
			a[1]=*S.top_imaginary;
            *S.top_real=(a[0]*b[0]+a[1]*b[1])/(b[0]*b[0]+b[1]*b[1]);
			*S.top_imaginary=(-1*a[0]*b[1]+a[1]*b[0])/(b[0]*b[0]+b[1]*b[1]);
            S.top_real++;
			S.top_imaginary++;
        }
        else
			...
			*S.top_imaginary=0;
			if (E->real[0]!='\0')
				for(int j=0;E->real[j]!='\0';j++)
					*S.top_real=*S.top_real*10+(double)(E->real[j]-48);
			else
				for(int j=0;E->imaginary[j]!='\0';j++)
					*S.top_imaginary=*S.top_imaginary*10+(double)(E->imaginary[j]-48);
            S.top_real++;
			S.top_imaginary++;
        }
		E=E->next;
	...
	S.top_real--;
	S.top_imaginary--;
	if (*S.top_imaginary>=0)
		printf("%f+%fj\n",*S.top_real,*S.top_imaginary);
	else
		printf("%f%fj\n",*S.top_real,*S.top_imaginary);
    free(S.base1);
	free(S.base2);
